...
TipoSimboloGramatical=(tProgram , tIdentificador , tConstanteReal , tConstanteEntera , tConstanteCadena , tParentesisAbre , tParentesisCierra , tMas
, tMenos , tProducto , tDivision, tPuntoYComa , tComa, tPunto , tOperadorRelacional , tAnd , tOr , tNot , tOperadorAsignacion ,
tBegin , tEnd , tIf , tThen , tElse , tWhile , tDo , tRead , tWrite , tRaiz , tCadena , tReal , error , pesos);
...
begin
     ...
     tablaSimbolos.elem[11].componenteLexico := tAnd;
     tablaSimbolos.elem[11].lexema := 'AND';
     tablaSimbolos.elem[12].componenteLexico := tOr;
     tablaSimbolos.elem[12].lexema := 'OR';
     tablaSimbolos.elem[13].componenteLexico := tNot;
...
begin
     ...
     enc := false;
     while (i <= tablaSimbolos.cant) and (enc = false) do //este ciclo recorre las palabras reservadas ya cargadas en la tabla
           begin
     ...
     if enc = false then    // si no se ha encontrado el lexema en la tabla con las palabras ya cargadas, agrega el par componenteLexico - lexema a la tabla
        begin
             inc(i);
             tablaSimbolos.elem[i].lexema := uppercase(lexemaLeido);
             tablaSimbolos.elem[i].componenteLexico := tIdentificador;
        end;
