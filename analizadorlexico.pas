...
Begin
     ...
     While (EstadoActual <> 2) and (EstadoActual <> 3) do //no sea el estado muerto ni final
           begin
                LeerCaracter(Fuente, ControlAux, Car);
                EstadoActual:=Delta[EstadoActual,caracterASimbolo(Car)];
                ControlAux:=ControlAux+1;
...
function caracterASimbolo(caracter:char):sigma;
begin
     if (caracter = '"') then  //tambien puede ser el ascii de comilla?
           ...
           if Caracter = '=' then
              begin
               ...
               begin
                     ...
                     Lexema := '<';
                     EsSimboloEspecial := true;
                     //inc(control); no se incrementa el control
               end;
      end;
    '>':
      begin
...
begin
     ...
     while caracter in [#1..#32] do //esta parte saltea los caracteres de control
           begin
                inc(control);
                LeerCaracter(fuente,control,caracter);
           end;
     ...
     if caracter = FinArchivo then
        begin
         ...
         begin
              if EsIdentificador(fuente,control,lexema) then                      //la tabla de simbolos ya tiene cargadas las palabras reservadas
